...
class SettingController extends Controller
{
	...
	public function risk_management_matchCallForFancyNBM($matchId, Request $request)
	{
		...
		$team1_bet_total=0;
		$team_draw_bet_total=0;
		$my_placed_bets = MyBets::where('match_id',$eventId)->where('bet_type','BOOKMAKER')->where('result_declare',0)->where('isDeleted',0)->get();
			...
			if(@$match_data['fancy'])
			{
				...
				for($i=0;$i<sizeof($sid);$i++)
				{
					...
					if($match_f == '0')
					{
						...
						//bet calculation
						$my_placed_bets = MyBets::where('match_id',$eventId)->where('team_name',@$nat[$sid[$i]])->where('bet_type','SESSION')->where('result_declare',0)->where('isDeleted',0)->orderBy('created_at', 'asc')->get();
						if(sizeof($my_placed_bets)>0)
						{
							$run_arr=array();
							foreach($my_placed_bets as $bet)
