    ...
    <div class="row">
      <div class="col-md-12 pl-0">
        <div class="downline-block">
          ...
          <ul class="agentlist">
            <li class="firstli"><a><span class="blue-bg text-color-white">{{$user->agent_level}}</span><strong>{{$user->user_name}}</strong></a> <img src="{{ URL::to('asset/img/arrow-right2.png')}}"> </li>                     
          </ul>
        </div>
      </div>
      ...
      <div class="col-lg-9 col-md-9 col-sm-12">
        ...
        <p class="d-flex algin-item-center mt-2 mb-0"> <img src="{{ URL::to('asset/img/user-icon-blue.png')}}" class="mr-1"> {{$user->user_name}}</p>
        ...
        <div class="white-bg mt-2 mb-3 acc-statement">
          <table class="table custom-table white-bg text-color-blue-2">
            <tbody>
              ...
              <tr>
                <td> Main wallet </td>
                <td> {{$userdata->available_balance_for_D_W}} </td>
                <td> {{$userdata->remain_bal}} </td>
                <td> {{$userdata->exposure}} </td>
              </tr>
            </tbody>
          </table>
        </div>
...
<div class="modal credit-modal changepwd-modal" id="mypwd">
  <div class="modal-dialog">
    <div class="modal-content light-grey-bg-1">
      ...
      <form method="post" action="{{route('updatePasswordadmin',$id)}}" id="agentform">
        @csrf
...
<script type="text/javascript">
	$('#btnpwd').click(function () {
		...
		$('#erryourpwd').html('');			
    if(yourpwd == ''){
      toastr.error('Your password can not be blank!');
      return false;
    }
		if(newpwd == ''){
			toastr.error('New password can not be blank!');
      return false;
		}
		if(newpwd !=''){
			if(newpwd.length < 8){	
